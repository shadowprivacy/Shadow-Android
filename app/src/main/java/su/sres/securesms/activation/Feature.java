...
import static su.sres.securesms.activation.License.BYTES;
...
public class Feature {
    ...
    private static final int BYTEBYTES = 1;
    private static final int SHORTBYTES = 2;
    private static final int LONGBYTES = 8;
    private static final int DOUBLEBYTES = 8;
    ...
    static String[] splitString(String s) {
        ...
        final int colIndex = s.substring(0, typeEnd).indexOf(":");
        final int nameEnd = colIndex == -1 ? typeEnd : colIndex;
    ...
    public byte[] serialized() {
        ...
        final int typeLength = BYTES;
        final int nameLength = BYTES + nameBuffer.length;
        final int valueLength = type.fixedSize == VARIABLE_LENGTH ? BYTES + value.length : type.fixedSize;
    ...
    public BigDecimal getBigDecimal() {
        ...
        int scale = bb.getInt(value.length - BYTES);

        return new BigDecimal(new BigInteger(Arrays.copyOf(value, value.length - BYTES)), scale);
    }
    ...
    private enum Type {
        BINARY(1, VARIABLE_LENGTH,
                ...
                ba -> Base64.encodeBytes((byte[]) ba), enc -> Base64.decodeOrThrow(enc)),
    ...
    public static class Create {
        ...
        public static Feature shortFeature(String name, Short value) {
            Objects.requireNonNull(value);
            return new Feature(name, Type.SHORT, ByteBuffer.allocate(SHORTBYTES).putShort(value).array());
        }
        ...
        public static Feature intFeature(String name, Integer value) {
            Objects.requireNonNull(value);
            return new Feature(name, Type.INT, ByteBuffer.allocate(BYTES).putInt(value).array());
        }
        ...
        public static Feature longFeature(String name, Long value) {
            Objects.requireNonNull(value);
            return new Feature(name, Type.LONG, ByteBuffer.allocate(LONGBYTES).putLong(value).array());
        }
        ...
        public static Feature floatFeature(String name, Float value) {
            Objects.requireNonNull(value);
            return new Feature(name, Type.FLOAT, ByteBuffer.allocate(FLOATBYTES).putFloat(value).array());
        }
        ...
        public static Feature doubleFeature(String name, Double value) {
            Objects.requireNonNull(value);
            return new Feature(name, Type.DOUBLE, ByteBuffer.allocate(DOUBLEBYTES).putDouble(value).array());
        }
        ...
        public static Feature uuidFeature(String name, java.util.UUID value) {
            Objects.requireNonNull(value);
            return new Feature(name, Type.UUID, ByteBuffer.allocate(2 * LONGBYTES).putLong(value.getLeastSignificantBits()).putLong(value.getMostSignificantBits()).array());
        }
        ...
        public static Feature dateFeature(String name, Date value) {
            Objects.requireNonNull(value);
            return new Feature(name, Type.DATE, ByteBuffer.allocate(LONGBYTES).putLong(value.getTime()).array());
        }
        ...
        public static Feature bigDecimalFeature(String name, BigDecimal value) {
            ...
            return new Feature(name, Type.BIGDECIMAL, ByteBuffer.allocate(BYTES + b.length)
                    .put(b)
                    .putInt(value.scale())
                    .array());
        }
