...
public final class AppStartup {
    ...
    private final List<Task> foremost;
    ...
    /**
     * Same as {@link #addBlocking(String, Runnable)}, but for tasks that must be executed in the network-independent phase.
     */
    @MainThread
    public @NonNull
    AppStartup addForemost(@NonNull String name, @NonNull Runnable task) {
        foremost.add(new Task(name, task));
        return this;
    }
    ...
    /**
     * Begins pending foremost tasks execution.
     */
    @MainThread
    public void executeForemost() {
        Stopwatch stopwatch = new Stopwatch("foremost");

        for (Task task : foremost) {
    ...
    /**
     * Begins pending non-foremost task execution.
     */
    public void execute() {
