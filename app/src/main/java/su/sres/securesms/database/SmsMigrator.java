...
public class SmsMigrator {
  ...
  {
    MessageDatabase ourSmsDatabase = ShadowDatabase.sms();
    Cursor          cursor         = null;
    ...
    try {
      ...
      ShadowDatabase.threads().update(ourThreadId, true);
      ShadowDatabase.threads().setLastScrolled(ourThreadId, 0);
      ShadowDatabase.threads().notifyConversationListeners(ourThreadId);
      ...
      while (cursor != null && cursor.moveToNext()) {
        ...
        if (ourRecipients != null) {
          if (ourRecipients.size() == 1) {
            ...
            GroupId.Mms ourGroupId          = ShadowDatabase.groups().getOrCreateMmsGroupForMembers(recipientIds);
            RecipientId ourGroupRecipientId = ShadowDatabase.recipients().getOrInsertFromGroupId(ourGroupId);
