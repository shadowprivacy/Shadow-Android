 ...
 */
class ReactionDatabase(context: Context, databaseHelper: ShadowDatabase) : Database(context, databaseHelper) {
  ...
  fun addReaction(messageId: MessageId, reaction: ReactionRecord) {
    ...
    try {
      ...
      if (messageId.mms) {
        ShadowDatabase.mms.updateReactionsUnread(db, messageId.id, hasReactions(messageId), false)
      } else {
        ShadowDatabase.sms.updateReactionsUnread(db, messageId.id, hasReactions(messageId), false)
      }
  ...
  fun deleteReaction(messageId: MessageId, recipientId: RecipientId) {
    ...
    try {
      ...
      if (messageId.mms) {
        ShadowDatabase.mms.updateReactionsUnread(db, messageId.id, hasReactions(messageId), true)
      } else {
        ShadowDatabase.sms.updateReactionsUnread(db, messageId.id, hasReactions(messageId), true)
      }
