...
import su.sres.securesms.database.helpers.ShadowDatabaseMigrations
import su.sres.securesms.database.helpers.ShadowDatabaseMigrations.migrate
...
open class ShadowDatabase(private val context: Application, databaseSecret: DatabaseSecret, attachmentSecret: AttachmentSecret) :
  SQLiteOpenHelper(
    context,
    DATABASE_NAME,
    databaseSecret.asString(),
    null,
    ShadowDatabaseMigrations.DATABASE_VERSION,
    0,
  ...
  open fun markCurrent(db: net.zetetic.database.sqlcipher.SQLiteDatabase) {
    db.version = ShadowDatabaseMigrations.DATABASE_VERSION
  }
  ...
  companion object {
    ...
    @JvmStatic
    @Volatile
    var instance: ShadowDatabase? = null
      private set

    @JvmStatic
    fun init(application: Application, databaseSecret: DatabaseSecret, attachmentSecret: AttachmentSecret) {
      if (instance == null) {
        synchronized(ShadowDatabase::class.java) {
          if (instance == null) {
            instance = ShadowDatabase(application, databaseSecret, attachmentSecret)
          }
        }
      }
    }
