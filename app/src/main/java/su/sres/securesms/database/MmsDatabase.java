    ...
    public void incrementReceiptCount(SyncMessageId messageId, long timestamp, boolean deliveryReceipt, boolean readReceipt) {
        ...
        try {
            ...
            if (!found) {
                ...
                if (readReceipt)
                    earlyReadReceiptCache.increment(messageId.getTimetamp(), messageId.getRecipientId());
            }
        } finally {
            if (cursor != null)
                cursor.close();
...
//  }
    ...
    public long insertMessageOutbox(@NonNull OutgoingMediaMessage message,
        ...
        if (message.getRecipient().isGroup()) {
            ...
            for (RecipientId recipientId : earlyReadReceipts.keySet())
                receiptDatabase.update(recipientId, messageId, GroupReceiptDatabase.STATUS_READ, -1);
        }
    ...
    private @Nullable
    String getSerializedSharedContacts(@NonNull Map<Attachment, AttachmentId> insertedAttachmentIds, @NonNull List<Contact> contacts) {
    ...
    private @Nullable
    String getSerializedLinkPreviews(@NonNull Map<Attachment, AttachmentId> insertedAttachmentIds, @NonNull List<LinkPreview> previews) {
