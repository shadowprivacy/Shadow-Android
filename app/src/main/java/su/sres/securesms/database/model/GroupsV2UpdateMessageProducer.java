...
import su.sres.securesms.groups.GV2AccessLevelUtil;
...
final class GroupsV2UpdateMessageProducer {
  ...
  private void describeInvitations(@NonNull DecryptedGroupChange change, @NonNull List<UpdateDescription> updates) {
    ...
    if (notYouInviteCount > 0) {
      ...
      updates.add(updateDescription(change.getEditor(), editor -> context.getResources()
                                                                         .getQuantityString(R.plurals.MessageRecord_s_invited_members, notYouInviteCountFinalCopy, editor, notYouInviteCountFinalCopy), R.drawable.ic_update_group_add_16));
    }
  }
  ...
  private void describeRevokedInvitations(@NonNull DecryptedGroupChange change, @NonNull List<UpdateDescription> updates) {
    ...
    if (notDeclineCount > 0) {
      if (editorIsYou) {
        ...
        updates.add(updateDescription(change.getEditor(), editor -> context.getResources()
                                                                           .getQuantityString(R.plurals.MessageRecord_s_revoked_invites, notDeclineCountFinalCopy, editor, notDeclineCountFinalCopy), R.drawable.ic_update_group_decline_16));
      }
    }
  }
