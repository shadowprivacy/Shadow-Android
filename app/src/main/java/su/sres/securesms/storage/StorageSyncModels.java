...
public final class StorageSyncModels {
  ...
  public static @NonNull SignalStorageRecord localToRemoteRecord(@NonNull RecipientSettings settings, @NonNull byte[] rawStorageId) {
    switch (settings.getGroupType()) {
      case NONE:
        return SignalStorageRecord.forContact(localToRemoteContact(settings, rawStorageId));
      case SIGNAL_V1:
        return SignalStorageRecord.forGroupV1(localToRemoteGroupV1(settings, rawStorageId));
      case SIGNAL_V2:
        return SignalStorageRecord.forGroupV2(localToRemoteGroupV2(settings, rawStorageId, settings.getSyncExtras().getGroupMasterKey()));
      default:
  ...
  public static AccountRecord.UserLoginSharingMode localToRemoteUserLoginSharingMode(UserLoginPrivacyValues.UserLoginSharingMode userLoginUserLoginSharingMode) {
    switch (userLoginUserLoginSharingMode) {
      case EVERYONE:
        return AccountRecord.UserLoginSharingMode.EVERYBODY;
      case NOBODY:
        return AccountRecord.UserLoginSharingMode.NOBODY;
      default:
  ...
  public static UserLoginPrivacyValues.UserLoginSharingMode remoteToLocalUserLoginSharingMode(AccountRecord.UserLoginSharingMode userLoginUserLoginSharingMode) {
    switch (userLoginUserLoginSharingMode) {
      case EVERYBODY:
        return UserLoginPrivacyValues.UserLoginSharingMode.EVERYONE;
      case NOBODY:
        return UserLoginPrivacyValues.UserLoginSharingMode.NOBODY;
      default:
        return UserLoginPrivacyValues.UserLoginSharingMode.NOBODY;
    }
  }
  ...
  private static @NonNull SignalAccountRecord.PinnedConversation localToRemotePinnedConversation(@NonNull RecipientSettings settings) {
    switch (settings.getGroupType()) {
      ...
      case SIGNAL_V2:
        return SignalAccountRecord.PinnedConversation.forGroupV2(settings.getSyncExtras().getGroupMasterKey().serialize());
      default:
