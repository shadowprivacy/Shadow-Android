...
import su.sres.signalservice.api.subscriptions.SubscriptionClientSecret;
...
public class PushServiceSocket {
                           ...
                           boolean automaticNetworkRetry)
  {
    ...
    this.serviceClients            = createServiceConnectionHolders(configuration.getSignalServiceUrls(), configuration.getNetworkInterceptors(), configuration.getDns(), configuration.getShadowProxy());
    this.cdnClientsMap             = createCdnClientsMap(configuration.getSignalCdnUrlMap(), configuration.getNetworkInterceptors(), configuration.getDns(), configuration.getShadowProxy());
    this.storageClients            = createConnectionHolders(configuration.getSignalStorageUrls(), configuration.getNetworkInterceptors(), configuration.getDns(), configuration.getShadowProxy());
  ...
  public void requestSmsVerificationCode(boolean androidSmsRetriever, Optional<String> captchaToken, Optional<String> challenge) throws IOException {
    String path = String.format(CREATE_ACCOUNT_SMS_PATH, credentialsProvider.getUserLogin(), androidSmsRetriever ? "android-2021-03" : "android");
  ...
  public AttachmentV2UploadAttributes getDebugLogUploadAttributes() throws NonSuccessfulResponseCodeException, PushNetworkException, MalformedResponseException {
    ...
    return makeServiceBodyRequest(urlFragment,
                                  ...
                                  new ResponseCodeHandler() {
                                    ...
                                    public void handle(int responseCode, Headers responseHeaders) throws NonSuccessfulResponseCodeException {
                                      ...
                                      if (cdHeader == null)
                                        throw new NonSuccessfulResponseCodeException(responseCode, String.format("Bad response: received %d from the server with no Content-Disposition header", responseCode));
                                      if (!contents.equals(cdHeader))
                                        throw new NonSuccessfulResponseCodeException(responseCode, String.format("Bad response: received %d from the server with invalid Content-Disposition header", responseCode));
                                    }
                                  },
                                  Optional.<UnidentifiedAccess>absent());
  }
...
//      Log.d(TAG, "Opening URL: " + String.format("%s%s", connectionHolder.getUrl(), urlFragment));
  ...
  private static Map<Integer, ConnectionHolder[]> createCdnClientsMap(final Map<Integer, SignalCdnUrl[]> signalCdnUrlMap,
                                                                      ...
                                                                      final Optional<ShadowProxy> proxy)
  {
  ...
  private static ConnectionHolder[] createConnectionHolders(SignalUrl[] urls, List<Interceptor> interceptors, Optional<Dns> dns, Optional<ShadowProxy> proxy) {
  ...
  private static OkHttpClient createConnectionClient(SignalUrl url, List<Interceptor> interceptors, Optional<Dns> dns, Optional<ShadowProxy> proxy) {
    try {
...
//    this.serviceClients                    = createServiceConnectionHolders(signalServiceConfiguration.getSignalServiceUrls(), signalServiceConfiguration.getNetworkInterceptors());
    this.cdnClientsMap  = createCdnClientsMap(signalServiceConfiguration.getSignalCdnUrlMap(), signalServiceConfiguration.getNetworkInterceptors(), signalServiceConfiguration.getDns(), signalServiceConfiguration.getShadowProxy());
    this.storageClients = createConnectionHolders(signalServiceConfiguration.getSignalStorageUrls(), signalServiceConfiguration.getNetworkInterceptors(), signalServiceConfiguration.getDns(), signalServiceConfiguration.getShadowProxy());
  }
